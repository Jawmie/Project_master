...
#define MY_DEV		((LPC_UART_TypeDef *)LPC_UART3)
 ...
 *
 * Description:
 *   Initialize UART#3 for sending Chars to bluetooth (In Theory)
 *
 *****************************************************************************/
void myblue_init(void)
{
	...
	PinCfg.Funcnum = 1; // Left the way it is
	PinCfg.Portnum = 4; // UART#3 is on port 4
	PinCfg.Pinnum = 28;  // pin 28 is TX
	PINSEL_ConfigPin(&PinCfg);
	PinCfg.Pinnum = 29;	 // pin 29 is RX
	PINSEL_ConfigPin(&PinCfg);
 ...
 *
 *****************************************************************************/
uint32_t myblue_send(uint8_t *txbuf, uint32_t buflen,
		TRANSFER_BLOCK_Type flag)
{
	return UART_Send(MY_DEV, txbuf, buflen, flag);
}
 ...
 *
 *****************************************************************************/
uint32_t myblue_sendString(uint8_t *str)
{
	return UART_SendString(MY_DEV, str);
}
/******************************************************************************
 *
 * Description:
